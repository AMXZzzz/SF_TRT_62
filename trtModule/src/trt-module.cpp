...
// 宏重载
#define TRTHEAD "[TRT]: "
#define LOGINFO(format,...) if (m_logger) {m_logger->info(TRTHEAD ## format, __VA_ARGS__);} 
#define LOGWARN(format,...)  if (m_logger) {m_logger->warn(TRTHEAD ## format, __VA_ARGS__);}
#define LOGERROR(format,...) if (m_logger) {m_logger->error(TRTHEAD ## format, __VA_ARGS__);}
...
IStates ITensorRt::getLastErrorInfo() {
	if (errors.empty()) {
		...
		return IStates(errors[idx].is_success(), errors[idx].getErrorInfo());
	}
}
