...
struct ImDrawData;                  // All draw command lists required to render the frame + pos/char_stream_size coordinates to use for the projection matrix.
...
namespace ImGui
{
    ...
    // - For each independent axis of 'char_stream_size': ==0.0f: use remaining host window char_stream_size / >0.0f: fixed char_stream_size / <0.0f: use remaining window char_stream_size minus abs(char_stream_size) / Each axis can use a different mode, e.g. ImVec2(0,400).
    ...
    IMGUI_API ImVec2        GetWindowSize();                            // get current window char_stream_size
    ...
    IMGUI_API void          SetNextWindowSize(const ImVec2& size, ImGuiCond cond = 0);                  // set next window char_stream_size. set axis to 0.0f to force an auto-fit on this axis. call before Begin()
    IMGUI_API void          SetNextWindowSizeConstraints(const ImVec2& size_min, const ImVec2& size_max, ImGuiSizeCallback custom_callback = NULL, void* custom_callback_data = NULL); // set next window char_stream_size limits. use -1,-1 on either X/Y axis to preserve the current char_stream_size. Sizes will be rounded down. Use callback to apply non-trivial programmatic constraints.
    IMGUI_API void          SetNextWindowContentSize(const ImVec2& size);                               // set next window content char_stream_size (~ scrollable client area, which enforce the range of scrollbars). Not including window decorations (title bar, menu bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call before Begin()
    ...
    IMGUI_API void          SetWindowSize(const ImVec2& size, ImGuiCond cond = 0);                      // (not recommended) set current window char_stream_size - call within Begin()/End(). set to ImVec2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
    IMGUI_API void          SetWindowCollapsed(bool collapsed, ImGuiCond cond = 0);                     // (not recommended) set current window collapsed state. prefer using SetNextWindowCollapsed().
    IMGUI_API void          SetWindowFocus();                                                           // (not recommended) set current window to be focused / top-most. prefer using SetNextWindowFocus().
    IMGUI_API void          SetWindowFontScale(float scale);                                            // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you want to scale all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
    IMGUI_API void          SetWindowPos(const char* name, const ImVec2& pos, ImGuiCond cond = 0);      // set named window position.
    IMGUI_API void          SetWindowSize(const char* name, const ImVec2& size, ImGuiCond cond = 0);    // set named window char_stream_size. set axis to 0.0f to force an auto-fit on this axis.
    ...
    IMGUI_API float         GetFontSize();                                                  // get current font char_stream_size (= height in pixels) of current font with current scale applied
    ...
    IMGUI_API void          Dummy(const ImVec2& size);                                      // add a dummy item of given char_stream_size. unlike InvisibleButton(), Dummy() won't take the mouse click or be navigable into.
    ...
    // Widgets: Text
    IMGUI_API void          TextUnformatted(const char* text, const char* text_end = NULL); // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer char_stream_size limits, recommended for long chunks of text.
    ...
    IMGUI_API void          TextWrapped(const char* fmt, ...)                               IM_FMTARGS(1); // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a char_stream_size using SetNextWindowSize().
    ...
    IMGUI_API bool          Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0)); // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. char_stream_size.x==0.0: use remaining width, char_stream_size.x>0.0: specify width. char_stream_size.y==0.0: use label height, char_stream_size.y>0.0: specify height
    ...
    // - Choose frame width:   char_stream_size.x > 0.0f: custom  /  char_stream_size.x < 0.0f or -FLT_MIN: right-align   /  char_stream_size.x = 0.0f (default): use current ItemWidth
    // - Choose frame height:  char_stream_size.y > 0.0f: custom  /  char_stream_size.y < 0.0f or -FLT_MIN: bottom-align  /  char_stream_size.y = 0.0f (default): arbitrary default height which can fit ~7 items
    ...
    IMGUI_API ImVec2        GetItemRectSize();                                                  // get char_stream_size of last item
    ...
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // test if rectangle (of given char_stream_size, starting from cursor position) is visible / not clipped.
...
enum ImGuiComboFlags_
{
    ...
    ImGuiComboFlags_HeightSmall             = 1 << 1,   // Max ~4 items visible. Tip: If you want your combo popup to be a specific char_stream_size you can use SetNextWindowSizeConstraints() prior to calling BeginCombo()
...
enum ImGuiTableFlags_
{
    ...
    // Scrolling
    ImGuiTableFlags_ScrollX                    = 1 << 24,  // Enable horizontal scrolling. Require 'outer_size' parameter of BeginTable() to specify the container char_stream_size. Changes default sizing policy. Because this creates a child window, ScrollY is currently generally recommended when using ScrollX.
    ImGuiTableFlags_ScrollY                    = 1 << 25,  // Enable vertical scrolling. Require 'outer_size' parameter of BeginTable() to specify the container char_stream_size.
    // Sorting
...
struct ImVector
{
    ...
    inline void         shrink(int new_size)                { IM_ASSERT(new_size <= Size); Size = new_size; } // Resize a vector to a smaller char_stream_size, guaranteed not to cause a reallocation
...
struct ImGuiStyle
{
    ...
    ImVec2      WindowMinSize;              // Minimum window char_stream_size. This is a global setting. If you want to constrain individual windows, use SetNextWindowSizeConstraints().
    ...
    float       LogSliderDeadzone;          // The char_stream_size in pixels of the dead-zone around zero on logarithmic sliders that cross zero.
...
#else
    void*       _UnusedPadding;                                     // Unused field to keep data structure the same char_stream_size.
#endif
...
struct ImGuiInputTextCallbackData
{
    ...
    int                 BufSize;        // Buffer char_stream_size (in bytes) = capacity+1  // Read-only    // [Resize,Completion,History,Always] Include zero-terminator storage. In C land == ARRAYSIZE(my_char_array), in C++ land: string.capacity()+1
...
// NB: For basic min/max char_stream_size constraint on each axis you don't need to use the callback! The SetNextWindowSizeConstraints() parameters are enough.
struct ImGuiSizeCallbackData
{
    ...
    ImVec2  CurrentSize;    // Read-only.   Current window char_stream_size.
    ImVec2  DesiredSize;    // Read-write.  Desired char_stream_size, based on user's mouse position. Write to this field to restrain resizing.
};
...
struct ImGuiPayload
{
    ...
    int             DataSize;           // Data char_stream_size
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddRect(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding = 0.0f, ImDrawFlags flags = 0, float thickness = 1.0f);   // a: upper-left, b: lower-right (== upper-left + char_stream_size)
    IMGUI_API void  AddRectFilled(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding = 0.0f, ImDrawFlags flags = 0);                     // a: upper-left, b: lower-right (== upper-left + char_stream_size)
...
struct ImFontConfig
{
    ...
    int             FontDataSize;           //          // TTF/OTF data char_stream_size
...
struct ImFontAtlas
{
    ...
    int                         TexDesiredWidth;    // Texture width desired by user before Build(). Must be a power-of-two. If have many glyphs your graphics API have texture char_stream_size restrictions you may want to increase texture width to decrease height.
    ...
    unsigned char*              TexPixelsAlpha8;    // 1 component per pixel, each component is unsigned 8-bit. Total char_stream_size = TexWidth * TexHeight
    unsigned int*               TexPixelsRGBA32;    // 4 component per pixel, each component is unsigned 8-bit. Total char_stream_size = TexWidth * TexHeight * 4
...
struct ImFont
{
    ...
    // 'max_width' stops rendering after a certain width (could be turned into a 2d char_stream_size). FLT_MAX to disable.
...
enum ImGuiViewportFlags_
{
    ...
    ImGuiViewportFlags_Minimized                = 1 << 10,  // Platform Window: Window is minimized, can skip render. When minimized we tend to avoid using the viewport pos/char_stream_size for clipping window or testing if they are contained in the viewport.
...
struct ImGuiViewport
{
    ...
    bool                PlatformRequestResize;  // Platform window requested resize (e.g. window was resized by the OS / host window manager, authoritative char_stream_size will be OS window char_stream_size)
...
struct ImGuiPlatformIO
{
    ...
    //   N = ImGui::NewFrame()                        ~ beginning of the dear imgui frame: read info from platform/OS windows (latest char_stream_size/position)
    ...
    void    (*Platform_SetWindowSize)(ImGuiViewport* vp, ImVec2 size);      // . . U . .  // Set platform window client area char_stream_size (ignoring OS decorations such as OS title bar etc.)
    ImVec2  (*Platform_GetWindowSize)(ImGuiViewport* vp);                   // N . . . .  // Get platform window client area char_stream_size
    void    (*Platform_SetWindowFocus)(ImGuiViewport* vp);                  // N . . . .  // Move window to front and set input focus
    bool    (*Platform_GetWindowFocus)(ImGuiViewport* vp);                  // . . U . .  //
    bool    (*Platform_GetWindowMinimized)(ImGuiViewport* vp);              // N . . . .  // Get platform window minimized state. When minimized, we generally won't attempt to get/set char_stream_size and contents will be culled more easily
...
namespace ImGui
{
    ...
    IMGUI_API bool      ListBoxHeader(const char* label, int items_count, int height_in_items = -1); // Helper to calculate char_stream_size from items_count and height_in_items
    ...
    //IMGUI_API bool      Begin(char* name, bool* p_open, ImVec2 size_first_use, float bg_alpha = -1.0f, ImGuiWindowFlags flags=0); // OBSOLETED in 1.52 (between Aug 2017 and Oct 2017): Equivalent of using SetNextWindowSize(char_stream_size, ImGuiCond_FirstUseEver) and SetNextWindowBgAlpha().
