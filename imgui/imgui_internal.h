...
// Helper: ImVec2ih (2D vector, half-char_stream_size integer, for long-term packed storage)
struct ImVec2ih
{
    short   x, y;
...
// We store the chunk char_stream_size first, and align the final char_stream_size on 4 bytes boundaries.
...
// ImDrawList: Lookup table char_stream_size for adaptive arc drawing, cover full circle.
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    float           FontSize;                   // Current/default font char_stream_size (optional, for simplified AddText overload)
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImVector<char>          TextA;                  // temporary UTF8 buffer for callbacks and other operations. this is not updated in every code-path! char_stream_size=capacity.
...
struct ImGuiWindowStackData
{
    ...
    ImGuiStackSizes         StackSizesOnBegin;      // Store char_stream_size of various stacks for asserting
};
...
enum ImGuiOldColumnFlags_
{
    ...
    ImGuiOldColumnFlags_GrowParentContentsSize  = 1 << 4,   // (WIP) Restore pre-1.51 behavior of extending the parent window contents char_stream_size but _without affecting the columns width at all_. Will eventually remove.
...
struct IMGUI_API ImGuiDockNode
{
    ...
    ImVec2                  Size;                       // Current char_stream_size
    ImVec2                  SizeRef;                    // [Split node only] Last explicitly written-to char_stream_size (overridden when using a splitter affecting the node), used to calculate Size.
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    // Calculate work rect pos/char_stream_size given a set of offset (we have 1 pair of offset for rect locked from last frame data, and 1 pair for currently building rect)
...
struct ImGuiContext
{
    ...
    ImVector<unsigned char> DragDropPayloadBufHeap;             // We don't expose the ImVector<> directly, ImGuiPayload only holds pointer+char_stream_size
    ...
    int                             TablesTempDataStacked;      // Temporary table data char_stream_size (because we leave previous instances undestructed, we generally don't use TablesTempData.Size)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  Size;                               // Current char_stream_size (==SizeFull or collapsed title bar char_stream_size)
    ...
    float                   WindowBorderSize;                   // Window border char_stream_size at the time of Begin().
    ...
    ImS8                    HiddenFramesCannotSkipItems;        // Hide the window for N frames while allowing items to be submitted so we can measure their char_stream_size
    ...
    ImVec2                  SetWindowPosVal;                    // store window position when using a non-zero Pivot (position set needs to be processed when we know the window char_stream_size)
    ...
    ImRect                  ContentRegionRect;                  // FIXME: This is currently confusing/misleading. It is essentially WorkRect but not handling of scrolling. We currently rely on it as right/bottom aligned sizing operation need some char_stream_size to rely on.
...
struct ImGuiTableColumn
{
    ...
    float                   ContentMaxXHeadersUsed;         // Contents maximum position for headers rows (regardless of freezing). TableHeader() automatically softclip itself + report ideal desired char_stream_size, to avoid creating extraneous draw calls
