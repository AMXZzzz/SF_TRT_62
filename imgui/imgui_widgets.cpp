...
bool ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Cannot use zero-char_stream_size for InvisibleButton(). Unlike Button() there is not way to fallback using the label char_stream_size.
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    // But we maintain a minimum char_stream_size in pixel to allow for the user to still aim inside.
...
// size_arg (for each axis) < 0.0f: align to end, 0.0f: auto, > 0.0f: specified char_stream_size
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    // Set position given a custom constraint (peak into expected window char_stream_size so we can position it)
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (!is_floating_point && v_range >= 0)                                     // v_range < 0 may happen on integer overflows
        grab_sz = ImMax((float)(slider_sz / (v_range + 1)), style.GrabMinSize); // For integer sliders: if possible have the grab char_stream_size represent 1 unit
...
static ImVec2 InputTextCalcTextSizeW(ImGuiContext* ctx, const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ...
    if (line_width > 0 || text_size.y == 0.0f)                        // whereas char_stream_size.y will ignore the trailing \n
        text_size.y += line_height;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        // We cannot test for 'backup_current_text_length != apply_new_text_length' here because we have no guarantee that the char_stream_size
        // of our owned buffer matches the char_stream_size of the string object held by the user, and by design we allow InputText() to be used
    ...
    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted char_stream_size
...
// (In C++ the 'float col[4]' notation for a function argument is equivalent to 'float* col', we only specify a char_stream_size to facilitate understanding of the code.)
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ImVec2 picker_size(g.FontSize * 8, ImMax(g.FontSize * 8 - (GetFrameHeight() + g.Style.ItemInnerSpacing.x), 1.0f)); // FIXME: Picker char_stream_size copied from main picker function
...
// FIXME: Selectable() with (char_stream_size.x == 0.0f) and (SelectableTextAlign.x > 0.0f) followed by SameLine() is currently not supported.
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Submit label or explicit char_stream_size to ItemSize(), whereas ItemAdd() will submit a larger/spanning rectangle.
    ImGuiID id = window->GetID(label);
    ...
    // We don't support (char_stream_size < 0.0f) in Selectable() because the ItemSpacing extension would make explicitly right-aligned sizes not visibly match other widgets.
...
// Tip: To have a list filling the entire window width, use char_stream_size.x = -FLT_MIN and pass an non-visible label e.g. "##empty"
// Tip: If your vertical char_stream_size is calculated from an item count (e.g. 10 * item_height) consider adding a fractional part to facilitate seeing scrolling boundaries (e.g. 10.25 * item_height).
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ...
    // Calculate char_stream_size from "height_in_items"
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    {
        // Calculate and set window char_stream_size/position
        ...
        // Report our char_stream_size into work area (for next frame) using actual window char_stream_size
    ...
    PushStyleVar(ImGuiStyleVar_WindowMinSize, ImVec2(0, 0)); // Lift normal char_stream_size constraint
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    {
        if (menu_is_open)
            menu_is_open = BeginPopupEx(id, window_flags); // menu_is_open can be 'false' when the popup is completely clipped (e.g. zero char_stream_size display)
        else
            g.NextWindowData.ClearFlags();          // we behave like Begin() and need to consume those values
        return menu_is_open;
    }
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        // and we cannot wait for the next BeginTabItem() call. We cannot compute this width within TabBarAddTab() because font char_stream_size depends on the active window.
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    // We use g.Style.FramePadding.y to match the square ArrowButton char_stream_size
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // Calculate tab contents char_stream_size
