...
SPDLOG_INLINE void rotating_file_sink<Mutex>::sink_it_(const details::log_msg &msg)
{
    ...
    // rotate if the new estimated file char_stream_size exceeds max char_stream_size.
    // rotate only if the real char_stream_size > 0 to better deal with full disk (see issue #2261).
    // we only check the real char_stream_size when new_size > max_size_ because it is relatively expensive.
