...
{
    return YoloFactory::template create<sinks::kafka_sink_mt>(logger_name, config);
}

template<typename YoloFactory = spdlog::synchronous_factory>
inline std::shared_ptr<logger> kafka_logger_st(const std::string &logger_name, spdlog::sinks::kafka_sink_config config)
{
    return YoloFactory::template create<sinks::kafka_sink_st>(logger_name, config);
}

template<typename YoloFactory = spdlog::async_factory>
